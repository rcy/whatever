...
func (c *ListCmd) Run(s *service.Service) error {
	...
	err := s.DB.Select(&events, `select * from events where aggregate_type = 'reminder' order by event_id asc`)
	if err != nil {
...
func (c *AddCmd) Run(s *service.Service) error {
	...
	err = s.InsertEvent("ReminderCreated", "reminder", ids.New(), payload)
	if err != nil {
...
func (c *DeleteCmd) Run(s *service.Service) error {
	...
	_, err = s.DB.Exec(`insert into events(aggregate_id, aggregate_type, event_type, event_data) values (?,?,?,?)`, aggID, "reminder", "ReminderDeleted", "{}")
	if err != nil {
...
func (c *UndeleteCmd) Run(s *service.Service) error {
	...
	_, err = s.DB.Exec(`insert into events(aggregate_id, aggregate_type, event_type, event_data) values (?,?,?,?)`, aggID, "reminder", "ReminderUndeleted", "{}")
	if err != nil {
		return fmt.Errorf("db.Exec: %w", err)
	}
